...
import { CldProvisioningDefaultError } from "../models/cld-provisioning-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | CldProvisioningDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<
        T,
        E | CldProvisioningDefaultError | ResponseValidationError
      >,
      raw: unknown,
    ]
  > {
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new CldProvisioningDefaultError(
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new CldProvisioningDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
